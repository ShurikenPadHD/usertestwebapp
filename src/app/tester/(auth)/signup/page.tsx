  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user && !data.session) {
        setError(null)
        setIsLoading(false)
        router.push('/tester/signin?message=Check your email to confirm your account')
        return
      }
      router.push('/tester/available')
      router.refresh()
    } catch {
    ...
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <Card variant="glass" className="p-8 bg-[#1a1a1a]/80 backdrop-blur-xl border border-white/10">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-bold mb-2">Become a tester</h1>
            <p className="text-gray-400">Join 10,000+ testers earning money</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm text-gray-300">Testing Experience</label>
              <div className="grid grid-cols-3 gap-3">
                {['Beginner', 'Intermediate', 'Expert'].map((exp) => (
                  <button
                    key={exp}
                    type="button"
                    onClick={() => setFormData({ ...formData, experience: exp.toLowerCase() })}
                    className={`p-3 rounded-xl border text-sm font-medium transition-all ${
                      formData.experience === exp.toLowerCase()
          ...
          <p className="text-center mt-6 text-gray-400">
            ...
            <Link href="/tester/signin" className="text-blue-400 hover:text-blue-300 font-medium transition-colors">
              Sign in
            </Link>
          </p>
        </Card>
